<template>
    <div class="navbar flex text-nord0 pr-2 py-5 items-center">
        <div class="logo flex items-center gap-4">
            <img src="/logo.png" alt="App logo" class="w-12 h-12">
        </div>
        <div class="quick_links mr-auto ml-24 text-gray-700 flex gap-16">
            <a href="/">Home</a>
            <a href="/download">Download</a>
            <a href="https://www.github.com/kyeboard/classroom/releases">Changelog</a>
            <a href="https://www.github.com/kyeboard/classroom">Source code</a>
        </div>
        <button class="bg-nord0 p-3 px-12 rounded-lg text-nord4">
            Download
        </button>
    </div>
</template>
